...
class NEUROMORPH_OT_length_in_space(bpy.types.Operator):
    """Length of line segment connecting two selected points in space"""
    bl_idname = "neuromorph.length_in_space"
...
class NEUROMORPH_OT_length_on_mesh(bpy.types.Operator):
    """Shortest path between two points through vertices on the mesh"""
    bl_idname = "neuromorph.length_on_mesh"
...
class NEUROMORPH_OT_length_edges(bpy.types.Operator):
    """Total length of all selected edges (or points)"""
    bl_idname = "neuromorph.length_edges"
...
def create_submesh_code():
    ...
    if obj_ptr.type=="MESH":
        ...
        # Create new mesh object, add it to scene
        new_name = bpy.context.object.string_name
        new_name_surf = new_name + "_surf"
        new_obj = add_new_mesh_to_scene(obj_ptr, new_name_surf, selected_verts, [], selected_faces)


        # Convert to triangles, if not already (necessary for volumne calculation)
        bpy.ops.object.mode_set(mode='OBJECT')
...
def add_new_mesh_to_scene(scene_obj, new_name, new_verts, new_edges, new_faces):
    ...
    new_obj.location = scene_obj.location
    new_obj.scale = scene_obj.scale
    new_obj.rotation_euler = scene_obj.rotation_euler
...
def make_original_active(obj_ptr, sel_inds):
    ...
    bpy.context.view_layer.objects.active = obj_ptr
    for ind in sel_inds:
...
class NEUROMORPH_OT_create_submesh(bpy.types.Operator):
    """Create surface area mesh as child of selected object"""
    bl_idname = "neuromorph.create_submesh"
...
class NEUROMORPH_OT_create_closed_submesh(bpy.types.Operator):
    """Create volume mesh as child of selected object"""
    bl_idname = "neuromorph.create_closed_submesh"
...
class NEUROMORPH_OT_create_both_meshes(bpy.types.Operator):
    """Create both surface area and volume meshes as children of selected object"""
    bl_idname = "neuromorph.create_both_meshes"
    bl_label = "create both new meshes at once"
    bl_options = {"REGISTER", "UNDO"}
    
    def execute(self, context):
        NEUROMORPH_OT_create_submesh.execute(self, context)
        NEUROMORPH_OT_create_closed_submesh.execute(self, context)
        return{'FINISHED'}


class NEUROMORPH_OT_create_whole_surfaces(bpy.types.Operator):
    """Measure all selected objects (volume and surface area of whole surfaces)"""
    bl_idname = "neuromorph.create_whole_surfaces"
    ...
    def execute(self, context):
        if bpy.context.object.mode == 'EDIT':  # if in edit mode, do nothing
           self.report({'INFO'},"Expecting multiple objects selected")
        else:
...
class NEUROMORPH_OT_adjust_scene_scale(bpy.types.Operator):
    """Rescale all objects in scene"""
    bl_idname = "neuromorph.adjust_scale"
    ...
    def execute(self, context):
        if bpy.ops.object.mode_set.poll():
            bpy.ops.object.mode_set(mode='OBJECT')
        active_in = bpy.context.view_layer.objects.active
        bpy.ops.object.select_all(action='DESELECT')
        ...
        for this_obj in bpy.data.objects:
            visibility_list.append(this_obj.hide_get())
            if this_obj.hide_get():
                this_obj.hide_set(False)
        ...
        for this_obj in bpy.data.objects:
            ...
            this_obj.hide_set(visibility_list[ind])
            ind += 1            

        bpy.context.view_layer.objects.active = active_in
        active_in.select_set(True)
        return{'FINISHED'}
...
class NEUROMORPH_OT_write_data(bpy.types.Operator, ExportHelper):
    """Write external file containing all measurements in the scene"""
    bl_idname = "neuromorph.write_data"
...
class NEUROMORPH_OT_write_selected_data(bpy.types.Operator, ExportHelper):
    """Write external file containing measurements of the selected objects"""
    bl_idname = "neuromorph.write_selected_data"
    bl_label = "Write data file"
...
class NEUROMORPH_PT_property_panel_geometry(bpy.types.Panel):
    bl_idname = "NEUROMORPH_PT_property_panel_geometry"
    bl_label = "NeuroMorph Geometry Properties"
...
class NEUROMORPH_PT_MeasurementToolsPanel(bpy.types.Panel):
    bl_idname = "NEUROMORPH_PT_MeasurementToolsPanel"
    ...
    def draw(self, context):
        ...
        if hasattr(obj, 'data'):
            ...
            col1 = split.column()
            col2 = split.column()
            col1.operator("neuromorph.create_submesh", text = "Surface Area")
            col2.operator("neuromorph.create_closed_submesh", text = "Volume")
        #     # row = layout.row()
            split = layout.split(factor=0.5, align=True)
            col1 = split.column()
            col2 = split.column()
            ...
            layout.label(text = len_str)
            layout.operator("neuromorph.length_in_space", text = "Distance Between 2 Points", icon="ARROW_LEFTRIGHT")
            layout.operator("neuromorph.length_edges", text="Length of Selected Edges", icon="CURVE_PATH")  # MESH_DATA, CURVE_PATH
            layout.operator("neuromorph.length_on_mesh", text = "Shortest Distance on Mesh", icon="CURVE_NCURVE")
        ...
        colR.operator("neuromorph.adjust_scale", text = "Apply Scale")

        layout.label(text = "-----Output Data-----")
        layout.operator("neuromorph.write_data", text='Export Measurements', icon='FILEBROWSER')
        layout.operator("neuromorph.write_selected_data", text = "Export Selected", icon='FILEBROWSER')
...
def register():
    ...
    bpy.types.Scene.float_scale = FloatProperty(name = "scale", default = 1.0, min = 10**-20, options = set())
...
classes = (
    NEUROMORPH_PT_MeasurementToolsPanel,
    NEUROMORPH_OT_adjust_scene_scale,
    NEUROMORPH_OT_write_data,
    NEUROMORPH_OT_write_selected_data,
    NEUROMORPH_PT_property_panel_geometry,
    NEUROMORPH_OT_length_in_space,
    NEUROMORPH_OT_length_edges,
    NEUROMORPH_OT_length_on_mesh,
    NEUROMORPH_OT_create_submesh,
    NEUROMORPH_OT_create_closed_submesh,
    NEUROMORPH_OT_create_both_meshes,
    NEUROMORPH_OT_create_whole_surfaces
)
