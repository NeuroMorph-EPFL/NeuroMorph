#    NeuroMorph_Centerline_Procesing.py (C) 2021,  Anne Jorstad
#
...
# Define the panel
class NEUROMORPH_PT_CenterlineProcessingPanel(bpy.types.Panel):
    bl_idname = "NEUROMORPH_PT_CenterlineProcessingPanel"
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator("neuromorph.preprocess_mesh", text='Clean Mesh for Processing', icon='SPHERE')

        layout.label(text = "---------- Get Centerline (3 Methods) ----------")
        # 3 options:  use vmtk, generate approximation via 2 clicked points, or creating something completely by hand
        
        split = layout.row().split(factor=0.1, align=True)  #percentage=0.1
        col1 = split.column()
        col2 = split.column()
        ...
        split1 = layout.row().split(factor=0.08, align=True)  # percentage=0.07
        col1a = split1.column()
        col2a = split1.column()
        col1a.label(text = "M1:")
        split = col2a.split(factor=0.5, align=True)  # percentage=0.5
        col1 = split.column()
        col2 = split.column()
        col1.operator("neuromorph.write_vtp", text='Write Mesh to vtp', icon='FILEBROWSER')
        col2.operator("neuromorph.load_centerline", text='Load Centerline from vtp', icon='FILEBROWSER')
        ...
        col1 = split.column()
        col2 = split.column()
        col1.operator("neuromorph.approx_centerline", text='Approximate Centerline', icon='MOD_CURVE')  # MOD_CURVE
        col2.operator("neuromorph.update_approx_centerline", text='Update Approx. Centerline', icon='MOD_CURVE')
        ...
        col2a = split1.column()
        col1a.label(text = "M3:")
        col2a.operator("neuromorph.redefine_centerline", text='Use Selected Centerline', icon='MOD_CURVE')

        # Functions depending on centerline object in Blender
        layout.label(text = "---------- Use Centerline ----------")
        ...
        row = layout.row()
        row.operator("neuromorph.get_surface_areas", text='Get Cross Sections', icon='ONIONSKIN_ON')  
                    # MOD_OPACITY, PROP_CON, ONIONSKIN_ON, HOLDOUT_ON, MOD_SUBSURF, CLIPUV_DEHLT, CLIPUV_HLT

        row = layout.row()
        row.operator("neuromorph.max_radii", text='Get Maximum Radius of each Cross Section', icon='SURFACE_NCURVE')  
                    # DRIVER_DISTANCE, SURFACE_NCURVE

        row = layout.row()
        row.operator("neuromorph.project_vesicles", text='Project Spheres to Centerline', icon="TRACKING_REFINE_FORWARDS")  
                    # TRACKING_REFINE_FORWARDS, FULLSCREEN_EXIT, ORIENTATION_NORMAL

        row = layout.row()
        row.operator("neuromorph.project_areas", text='Project Surface Areas to Centerline', icon="TRACKING_REFINE_FORWARDS")

        row = layout.row()
        row.operator("neuromorph.write_ctrline_data", text='Write Centerline Data', icon='FILEBROWSER')


        layout.label(text = "---------- Detect Boutons ----------")
        split = layout.row().split(factor=0.1, align=True)  # percentage=0.1
        col1 = split.column()
        col2 = split.column()
        ...
        row = layout.row()
        row.operator("neuromorph.detect_boutons", text='Detect Boutons', icon="NODE_MATERIAL")  # NODE_MATERIAL, SHADING_RENDERED

        row = layout.row()
        row.operator("neuromorph.select_volume", text='Get Axon Region Between Points', icon="CURVE_DATA")  # PIVOT_INDIVIDUAL, CURVE_DATA


        # # self.layout.label(text = "--Debugging--") 
        # # row = self.layout.row()
        # # row.operator("neuromorph.separate_spheres", text='Separate Vesicle Spheres')
        # # row = self.layout.row()
        # # row.operator("neuromorph.add_radii_spheres", text='Add radii spheres')
...
class NEUROMORPH_OT_approx_centerline(bpy.types.Operator):
    """Heuristic to provide approximate centerline (input mesh object with two endpoints selected)"""
    bl_idname = "neuromorph.approx_centerline"
    ...
    def execute(self, context):
        ...
        #         mesh.data.vertices[ind].select_set(True)
        # ################################################################################
        ...
        # Define material for cross section slices
        mat = bpy.data.materials.new("cross_section_material")
        mat.diffuse_color = (0.0,1.0,1.0,1.0)
        ...
        for ii in range(1,ninds-1):
            ...
            if result == False:  # Segment does not intersect cross section, move midpoint inside
                ...
                inspect_locs = False
                if inspect_locs:
                    bpy.ops.mesh.primitive_uv_sphere_add(radius = .01, location = ctr_prev)
                    bpy.ops.mesh.primitive_uv_sphere_add(radius = .01, location = ctr_next)
...
#                          return the midpoint of this segment
    ...
    if inspect_new_locs:
        bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = v_co)
        ...
        if opt == "return_edge_pt":
            ...
            if opt == "use_perimeter":
                ...
                if (result == False or dot_angle < 0):  # Use negative normal
                    ...
                    # bpy.ops.mesh.primitive_uv_sphere_add(radius = .01, location = closest_csvert_co)
                    # bpy.ops.mesh.primitive_uv_sphere_add(radius = .01, location = closest_csvert_co - .1*Vector(norm_dir))
            ...
            if inspect_new_locs:  # add spheres on either side of new point
                bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = closest_csvert_co)
                bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = furthest_csvert_co)
                bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = midpt_co)
...
# If v0 and v2 aren't reliable, could use average locations of v0/v-1 and v2/v3
    ...
    if inspect_new_vert_loc:  # add spheres on either side of new point
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = loc1)
        bpy.ops.mesh.primitive_uv_sphere_add(radius = .02, location = loc2)
...
class NEUROMORPH_OT_update_approx_centerline(bpy.types.Operator):
    """After you have adjusted the approximate centerline by hand, add more points and smooth"""
    bl_idname = "neuromorph.update_approx_centerline"
...
class NEUROMORPH_OT_select_volume(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.object.mode_set(mode='OBJECT')
        selected_obs = [ob for ob in bpy.context.scene.objects if ob.select_get() == True]
        ...
        bool_mod.object = box
        bpy.ops.object.modifier_apply(modifier = 'modifier1')  # apply_as='DATA'
...
class NEUROMORPH_OT_detect_boutons(bpy.types.Operator):
    ...
    bl_idname = "neuromorph.detect_boutons"
    ...
    def execute(self, context):
        ...
        if len(areas) == 0:
            infostr = "No cross sections detected with this centerline, must get cross sections first"
            self.report({'INFO'}, infostr)
            return {'FINISHED'}
        ...
        bpy.context.scene.collection.objects.link(sphere_parent)
        sphere_parent.parent = centerline
        update_collection_of_new_obj(sphere_parent, centerline)
...
def get_max_rad(centerline):
    ...
    csa_names = [elt for elt in children if elt.name[0:13] == "cross-section"]
    max_rads = []
...
class NEUROMORPH_OT_max_radii(bpy.types.Operator):
    """Calculate the maximum radius of each cross section (input: centerline object)"""
    bl_idname = "neuromorph.max_radii"
...
class NEUROMORPH_OT_preprocess_mesh(bpy.types.Operator):
    """Remove and fill non-manifold mesh geometry of selected mesh"""
    bl_idname = "neuromorph.preprocess_mesh"
...
class NEUROMORPH_OT_redefine_centerline(bpy.types.Operator):
    ...
    bl_idname = "neuromorph.redefine_centerline"
...
class NEUROMORPH_OT_get_surface_areas(bpy.types.Operator):
    """Get cross sections from selected centerline and axon (input: 2 objects)"""
    bl_idname = "neuromorph.get_surface_areas"
    bl_label = "Get cross sections from selected centerline and axon"
...
# Get area of intersection of plane with mesh
    ...
    #             cross_section.data.vertices[ind].select_set(True)
    #         bpy.ops.object.mode_set(mode='EDIT')
    #         bpy.ops.mesh.select_all(action='INVERT')
    #         bpy.ops.mesh.delete(type='VERT')
    #         bpy.ops.object.mode_set(mode='OBJECT')
...
class NEUROMORPH_OT_add_radii_spheres(bpy.types.Operator):
    """Add radii spheres to selected centerline"""
    bl_idname = "neuroorph.add_radii_spheres"
...
def add_sphere_at_loc(loc, rad, mat, parent):
    ...
    if parent != []:
        obj.parent = parent
        update_collection_of_new_obj(obj, parent)
...
def construct_curve(verts, name, collection_obj):
    edges = [[ii,ii+1] for ii in range(0,len(verts)-1)]
    ctrline_obj = add_curve_to_scene(collection_obj, name, verts, edges)
    return(ctrline_obj)


def add_curve_to_scene(collection_obj, new_name, new_verts, new_edges):
    # The collecton_obj is only used to define the collection of the new object
    ...
    # new_obj.location = collection_obj.location
    # new_obj.scale = collection_obj.scale
    # new_obj.rotation_euler = collection_obj.rotation_euler

    bpy.context.scene.collection.objects.link(new_obj)
    new_mesh.from_pydata(new_verts, new_edges, [])
    new_mesh.update(calc_edges=True)

    # Move new object to correct collection(s), for Blender 2.8
    these_collections = collection_obj.users_collection  # could be multiple
    default_collections = new_obj.users_collection
    for con in default_collections:
        con.objects.unlink(new_obj)
    for con in these_collections:
        con.objects.link(new_obj)
...
class NEUROMORPH_OT_separate_spheres(bpy.types.Operator):
    """Separate Spheres of selected object"""
    bl_idname = "neuromorph.separate_spheres"
...
def get_vesicle_list(vesicle_obj):
    ...
    bpy.ops.object.mode_set(mode='OBJECT')
    # bpy.ops.object.select_all(action='DESELECT')
    # bpy.context.view_layer.objects.active = vesicle_obj
    # vesicle_obj.select_set(True)
    ...
    # vesicle_list = [ob.name for ob in bpy.context.scene.objects if ob.select_get() == True]  # replaces original vesicle_obj
    return (vesicle_list)
...
class NEUROMORPH_OT_write_vtp(bpy.types.Operator, ExportHelper):
    """Write selected mesh to vtp file format"""
    bl_idname = "neuromorph.write_vtp"
    bl_label = "Write selected mesh to vtp file format"

    filename_ext = ".vtp"  # ExportHelper mixin class uses this
    def execute(self, context):
...
class NEUROMORPH_OT_load_centerline(bpy.types.Operator, ImportHelper):
    """Load centerline from vtp file"""
    bl_idname = "neuromorph.load_centerline"
    bl_label = "Load centerline from vtp file"

    filter_glob: StringProperty(
        default = "*.vtp",  # Define required filetype
        options = {'HIDDEN'},
        maxlen = 255,  # Max internal buffer length, longer would be clamped.
    )

    def execute(self, context):
        collection_obj = bpy.context.object  # used to assign collection of new curve later
        if not collection_obj:
            activate_an_object()
            collection_obj = bpy.context.object

        bpy.ops.object.mode_set(mode='OBJECT')  # else is error at end

        this_filename = self.filepath
        ...
        if subsample_curve:
            ...
            ctrline_obj = construct_curve(verts_sub, "centerline", collection_obj)
            ...
            ctrline_obj = construct_curve(verts, "centerline", collection_obj)
        ...
        bpy.context.view_layer.objects.active = ctrline_obj
        ctrline_obj.select_set(True)
...
class NEUROMORPH_OT_write_ctrline_data(bpy.types.Operator, ExportHelper):
    """Write all calculated data of selected centerline"""
    bl_idname = "neuromorph.write_ctrline_data"
...
classes = (
    NEUROMORPH_PT_CenterlineProcessingPanel,
    NEUROMORPH_OT_preprocess_mesh,
    NEUROMORPH_OT_write_vtp,
    NEUROMORPH_OT_load_centerline,
    NEUROMORPH_OT_approx_centerline,
    NEUROMORPH_OT_update_approx_centerline,
    NEUROMORPH_OT_redefine_centerline,
    NEUROMORPH_OT_get_surface_areas,
    NEUROMORPH_OT_max_radii,
    NEUROMORPH_OT_project_vesicles,
    NEUROMORPH_OT_project_areas,
    NEUROMORPH_OT_write_ctrline_data,
    NEUROMORPH_OT_detect_boutons,
    NEUROMORPH_OT_select_volume,
    NEUROMORPH_OT_add_radii_spheres,
    NEUROMORPH_OT_separate_spheres
)
